...
describe('UserOrIpThrottlerGuard.getTracker', () => {
  const guard = Object.create(UserOrIpThrottlerGuard.prototype);
  ...
  it('falls back to IP when not authenticated', async () => {
    const tracker = await (guard as any).getTracker({ ip: '198.51.100.7' });
