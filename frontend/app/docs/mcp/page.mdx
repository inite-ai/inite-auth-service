...
## Onboarding — `/v1/mcp/login`

INITE ships a public MCP endpoint at **`https://auth.inite.ai/v1/mcp/login`** so AI assistants can sign a user in without dropping them to a terminal. It speaks JSON-RPC 2.0 over plain HTTP, requires no `Authorization` header, and exposes three tools:

| Tool | Returns |
| --- | --- |
| `start_login` | `device_code`, `user_code`, `verification_uri_complete`, `expires_in`, `interval` — RFC 8628 step 1 |
| `poll_login` | `{ status: "pending" \| "approved" \| "denied" \| "expired" }` and on approval also `access_token`, `refresh_token`, `expires_in` |
| `whoami` | resolved user (from `Authorization: Bearer` header or `access_token` arg), or `{ authenticated: false }` |
...
User: log me in
AI:   [start_login]
       → "Open https://auth.inite.ai/v1/oauth/device?user_code=PDMT-JMBK
          in your browser and approve."
User: done
AI:   [poll_login(device_code)]
       → { status: "approved", access_token: "eyJ…" }
       → "Signed in. Add this token as Authorization: Bearer in your
          other MCP servers."
```

Every per-vertical MCP route also returns a `login_url` hint in its 401 body (and a `WWW-Authenticate: Bearer realm=…` header per RFC 6750), so any MCP client that hits a vertical without a token automatically learns where to onboard:

```json
{
  "error": "Unauthorized",
  "login_url": "https://auth.inite.ai/v1/mcp/login",
  "login_hint": "Run start_login on the login MCP server, complete the device flow, then resend this request with Authorization: Bearer <access_token>."
}
```

### Claude Desktop config — two servers

```json
{
  "mcpServers": {
    "inite-login": {
      "url": "https://auth.inite.ai/v1/mcp/login"
    },
    "inite-rent": {
      "url": "https://inite.rent/mcp/inite-rent",
      "headers": {
        "Authorization": "Bearer ${INITE_TOKEN}"
      }
    }
  }
}
...
After running `start_login` + `poll_login` on the first server, export the returned `access_token` as `INITE_TOKEN` (or paste into the second server's headers manually) and restart Claude. The token is universal — same JWT works against every vertical's `/mcp/<slug>` route because they all JWKS-verify against `auth-api.inite.ai`.

### CLI shortcut — `curl … login.sh | bash`

For terminal users:

```bash
curl -fsSL https://auth.inite.ai/login.sh | bash
# token cached at ~/.config/inite/auth.json
export INITE_TOKEN=$(jq -r .access_token ~/.config/inite/auth.json)
```

Same device flow under the hood — `/login.sh` and `/v1/mcp/login` share the public `inite-cli` OAuth client.
